    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.rect.collidepoint(event.pos):
                ...
                self.active = False
            self.color = AZUL if self.active else BLANCO
...
input_box1 = InputBox(270, 270, 32, 32)  
input_box1.update()
input_box1.draw(lcd)  
...
def connection():
    global raiz, configurar, salirMainScrn, botones, botonesConfig, salirGrabacionCSV, grabandoCSV, ip, servidorThread, newCommand, s, c, servidor, extActual, extActual2, videoCon, grabarVideo, grabandoVideo, archivoDatosNuevo, archivoVideoNuevo
...
def botonesPantallaConfiguracion():
    global pantallaConfiguracion, s, ip, servidorThread, servidor, videoCon, extActual, extActual2, root, raiz, salirGrabacionCSV, grabarVideo, botones, configurar, salirMainScrn, confiMax, MAXTEMP, MINTEMP, confiMin, salir, confiCal, c, comSerial
    if botonesConfig[0]['selected']:
        ...
        if not comSerial:
            ...
            threading.Thread(target=comunicacionSerial).start()            
...
def configuracionPantalla():
    global salirMainScrn, configurando, click, lcd, input_box1, pantallaConfiguracion
    configurando = True
    salirMainScrn = True
    while configurar:
        if click:
...
def comunicacionSerial():
    global configurar, comSerial, pantallaConfiguracion, lcd, input_box1, salir
    clock = pygame.time.Clock()
    while configurar and not salir:
        if comSerial:
            for event in pygame.event.get():
