...
flags = pygame.DOUBLEBUF | pygame.RESIZABLE | pygame.SRCALPHA
...
def dibujar_botones_configuracion():
    for boton in botonesConfig:
        ...
        lcd.blit(boton['imagen'], boton['rect'])
        textLabel = textFont_temp.render(boton['texto'][0], True, NEGRO)
...
def botonesPantallaConfiguracion():
    global root, raiz, salirGrabacionCSV, grabarVideo, botones, configurar, salirMainScrn, confiMax, MAXTEMP, MINTEMP, confiMin, salir, confiCal
...
def grabar_csv():
    ...
    filename = raiz +"dataFiles/camara_termica.csv"
    if not raiz + "dataFiles/camara_termica" + str(x_file_csv) + ".csv" in glob.glob(raiz +"dataFiles/*.csv"):
        filename =raiz + "dataFiles/camara_termica" + str(x_file_csv) + ".csv"
    else:
        x_file_csv += 1
        grabar_csv()
