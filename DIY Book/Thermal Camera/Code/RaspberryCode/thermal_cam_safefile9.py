...
def connection():
    global raiz, configurar, salirMainScrn, botones, botonesConfig, salirGrabacionCSV, grabandoCSV, ip, servidorThread, newCommand, s, c, servidor, extActual, extActual2, videoCon, grabarVideo, grabandoVideo
        ...
        try:
            ...
            while not salir:
                ...
                if newCommand == 'PNG':
                    ...
                    while nuevaRaiz:
                        c.send(str.encode('Raiz es: ' + raiz +'\n\r Desea cambiarla?'))
                        newCommand = (c.recv(1024))
                        if newCommand == 'SI':
                            c.send(str.encode('Ingrese nueva raiz'))
                            raiz = (c.recv(1024))
                        elif newCommand == 'NO':
                            nuevaRaiz = False
